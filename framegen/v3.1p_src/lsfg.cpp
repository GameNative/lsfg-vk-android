...
void LSFG_3_1P::initializeExternal(PFN_vkGetInstanceProcAddr gipa,
...
int32_t LSFG_3_1P::createContextFromImages(
...
std::vector<LSFG_3_1P::DebugImage> LSFG_3_1P::debugInternalImages(int32_t id) {
    std::vector<DebugImage> out;
    auto it = contexts.find(id);
    if (it == contexts.end()) return out;
    for (const auto& [name, img] : it->second.debugInternalImages())
        out.push_back({ name, img->handle(), img->getExtent(), img->getFormat() });
    return out;
}
#endif
